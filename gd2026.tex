...
Figure~\ref{fig:collapse}. The sleeve passes through triangles whose
vertices lie on padded obstacle boundaries, and near the source and
target, where the funnel is anchored to a single point, vertices of
the source or target obstacle that appear on the funnel's left and
right chains can wrap a chain around the obstacle and force the path
to detour around its corners. We eliminate these detours by
