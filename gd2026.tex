...
PageRank~\cite{page1999pagerank} on~$G$ once, and sort all nodes by
descending PageRank.

For level $0 \le z < Z$ we form a new graph $G_z$ as follows. Let
$T_z$ be the prefix of length $\lceil |V|/2^{Z-z}\rceil$ of the
PageRank-sorted node array, and let $S_z = 2^{Z-z}$ be the per-level
display-scale cap.
We process $T_z$ in rank-descending order. Each
