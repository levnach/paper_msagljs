...
%% Library name: anonymized as GJS under the anonymous class option,
%% rendered as MSAGLJS for camera-ready. Resolved into a constant macro
%% so it can flow into hyperref's pdftitle metadata.
...
%% Source-code URLs:
%% - In anonymous review mode, point at the anonymous.4open.science mirror
%%   so reviewers can browse the implementation.
%% - In camera-ready mode, point at the canonical public repository.
\newcommand{\anonsource}{https://anonymous.4open.science/r/gjs-163F/}
\newcommand{\canonsource}{https://github.com/microsoft/msagljs}

\ifthenelse{\boolean{gdanonymous}}{}{\supplement{\url{\canonsource}}}

\GAIDecl{Generative AI was used throughout the preparation of this article and the accompanying software, including writing and refactoring source code, generating figures, drafting and editing technical prose, and running tests and benchmarks. The authors reviewed all AI-assisted content and take responsibility for the final form.}
...
%% Helper for URLs that identify the authors/organization.
%% Renders the URL normally for camera-ready, and an "omitted" placeholder
%% under the anonymous class option. Toggle by adding/removing "anonymous"
%% in the \documentclass options at the top of this file.
\newcommand{\anonurl}[1]{\ifthenelse{\boolean{gdanonymous}}{\textit{[URL omitted for double-blind review]}}{\url{#1}}}

%% Helper for URLs to live demos that DO have an anonymous mirror available
%% during review. Argument 1 is the anonymous-deployment URL (e.g. on
%% Surge.sh or Netlify); leave it empty until the anon deploy exists, in
%% which case the macro falls back to the standard "[URL omitted]"
%% placeholder. Argument 2 is the canonical public URL used in the
%% camera-ready version.
\newcommand{\anondemowebgl}{https://gjs-sleeve.surge.sh/}% Set to anon deploy URL of the WebGL demo.
\newcommand{\anondemosvg}{}%   Set to anon deploy URL of the SVG demo.
...
demo can be seen at
\anondemourl{\anondemowebgl}{https://microsoft.github.io/msagljs/renderer-webgl/index.html}.
...
\texttt{@deck.gl/geo-layers} package, giving smooth pan and zoom. Live
demos are available online: a WebGL renderer for large
graphs\footnote{\anondemourl{\anondemowebgl}{https://microsoft.github.io/msagljs/renderer-webgl/index.html}}
and an SVG renderer for smaller, editable
graphs\footnote{\anondemourl{\anondemosvg}{https://microsoft.github.io/msagljs/renderer-svg/index.html}}.
...
several routes travel tight together inside a common sleeve;
representing each such stretch as a single clip with an attached edge
...
reduce the number elements in a tile, all clips within a tile that
share the same, or almost the same, pair of endpoints are bundled into
a single clip whose attached edge list accumulates every contributing
edge; the bundled geometry is taken from the first such clip
encountered during splitting, and the choice is consistent within each
level. Table~\ref{tab:max-per-tile} reports the resulting maximum
...
The CDT alone is already a constant-stretch spanner of the visibility
graph of its constraint set: the shortest path along CDT edges between
two mutually visible vertices is at most $4\pi\sqrt{3}/9 \approx 2.42$
times the visibility-geodesic distance~\cite{bose2006stretch}. Funnel
optimization on the sleeve can only further shorten the path, and
returns the visibility-geodesic exactly whenever the chosen sleeve
contains it.
...
cover of~$H$. Minimum vertex cover is NP-hard, so we approximate it
...
buckets the procedure runs in linear time, and is the same
source-side reduction used in many-to-many shortest-path
frameworks~\cite{knopp2007many}. Each demand edge is then routed
from whichever endpoint lies in~$R$. Applying this heuristic
...
composers graph with 3\,405 nodes and 13\,832 edges, shortening the
routing pass by $\sim$28\,\% and $\sim$31\,\% respectively.
...
Dijkstra over the polygon corners and the node centres. This is a
...
To quantify what the tile pyramid buys at view time, we run an
ablation against a single-layer baseline: the same pipeline is built
once with the full pyramid, $Z$~chosen as in
Table~\ref{tab:perf}, and once with $Z=0$, that is, a single root
tile holding the entire graph. After the build, a scripted tour zooms
the viewport in by $50$\,\% and pans it sinusoidally for six seconds
in headless Chrome on the same M4~Max laptop as
Tables~\ref{tab:perf} and \ref{tab:routing-modes}. A
\texttt{PerformanceObserver} captures main-thread long tasks above
$50$\,ms during the tour, and a probe walks deck.gl's Tileset2D at
peak zoom and sums the elements of the visible tiles, that is, nodes,
curve clips, labels, and arrowheads, as a proxy for the per-frame
GPU load. Table~\ref{tab:smoothness} reports the result, averaged
over two trials.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrrrrrr@{}}
    \toprule
    & & & \multicolumn{3}{c}{Pyramid} & \multicolumn{1}{c}{Single layer} & \\
    \cmidrule(lr){4-6}\cmidrule(lr){7-7}
    Graph & $|V|$ & $|E|$ & LT & LT total (ms) & Elts/frame & Elts/frame & Ratio \\
    \midrule
    gameofthrones &     407 &   2\,639 & 2 & 146 &  2\,647 &   4\,012 &   1.5$\times$ \\
    composers     &  3\,405 &  13\,832 & 4 & 306 &      36 &  41\,871 &  1163$\times$ \\
    ca-GrQc       &  5\,242 &  28\,968 & 3 & 174 &      10 &  78\,358 & 7836$\times$ \\
    ca-HepPh      & 12\,008 & 236\,978 & --- & --- & --- & 426\,115 & --- \\
    \bottomrule
  \end{tabular}
  \caption{Browsing-smoothness ablation. \textbf{Pyramid} is the
    \msagljs{} default, with $Z$ as in Table~\ref{tab:perf};
    \textbf{Single layer} caps the build at $Z=0$ and keeps the
    entire graph in a single root tile. \emph{LT}, number of
    main-thread long tasks above $50$\,ms during the six-second
    zoom-pan tour; \emph{LT total}, their total wall-clock duration in
    milliseconds; \emph{Elts/frame}, the number of elements,
    that is, nodes, curve clips, labels, and arrowheads, in the
    tiles selected for the viewport by deck.gl's Tileset2D at peak
    zoom; \emph{Ratio}, single-layer over pyramid elements per frame.
    All single-layer trials record zero long tasks: a single root
    tile has nothing to fetch during the tour. On \texttt{ca-HepPh}
    the in-browser pyramid build does not finish within the harness
    limit; Table~\ref{tab:perf} reports a $57.71$\,s end-to-end build
    for the same graph without the WebGL renderer attached.}
  \label{tab:smoothness}
\end{table}

Two effects dominate. First, in pyramid mode the tour triggers a
small, bounded amount of main-thread work---two to four long tasks
totalling $150$--$310$\,ms over the six-second tour---as deck.gl
materializes the tiles that the viewport enters; in single-layer mode
no new tiles arrive, so no long tasks are recorded. Second, and far
more important on real hardware, the pyramid reduces the per-frame
visible-element count by $1.5\times$ on the small Game of Thrones
graph and by three to four orders of magnitude on
\texttt{composers} and \texttt{ca-GrQc}. The single-layer alternative
must re-emit the entire graph on every frame: on \texttt{ca-HepPh}
that means $426{,}115$ elements per frame, well beyond what current
browser GPU pipelines can sustain at interactive rates. The pyramid's
small CPU overhead during navigation is therefore the price for an
asymptotically lighter rendering loop.
