...
To check route quality, on Game of Thrones we ran the \msagljs{}
spline router on the same layout. Both routers finalize their paths
with the funnel algorithm and differ only in how they choose the
channel: the spline router uses shortest paths on a Yao-graph spanner
of the visibility graph~\cite{dwyer2010fast}, the sleeve router uses
the Dijkstra-tree search on the CDT dual of
Section~\ref{sec:sleeve}. The sleeve router yields a total edge length
of $647.6$\,k arc-length units; the spline router yields $626.6$\,k, a
ratio of $1.03\times$. Both routers can pick suboptimal channels, but the
sleeve router does so more often than the spline router; the resulting
routes are within four percent of the spline router in total length.
