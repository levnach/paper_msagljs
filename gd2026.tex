...
window. Within each level, the displayed nodes are a subset of the
PageRank prefix at the adaptive scales chosen for that level---a
high-ranked candidate is dropped when it overlaps an already-accepted
node---and the displayed edges are the geodesics around exactly those
obstacles, computed by the sleeve router of
Section~\ref{sec:sleeve-routing}, and not a subsampling of finer
routes.
