...
\paragraph{Future work.} The current pipeline routes every edge twice, once on the whole graph and once per level after PageRank filtering, and completes the full pyramid before the first tile is shown to the user. For large graphs this preprocessing dominates the time-to-first-frame. A natural next step is to build the pyramid \emph{progressively}: produce the coarsest level first, hand it to the renderer, and compute finer levels in the background while the user pans and zooms the coarse level, so that by the time the user zooms in far enough to need the next level, that level is already in the cache. The PageRank prefix and the per-level sleeve rerouting both fit this streaming construction, since each level depends only on its own node prefix and obstacle set.
