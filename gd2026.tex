...
\While{some tile in $\mathit{levels}[z]$ exceeds $\mathcal{C}$ elements}
  ...
  \ForAll{tiles $T \in \mathit{levels}[z]$} \Comment{per-tile inner loop}
    \State split $T$ into its four sub-tiles of $\mathit{next}$ \Comment{Section~\ref{sec:tile-build}}
...
\For{$z = Z-1$ \textbf{down to} $0$}
  ...
  \State rebuild the tiles of $\mathit{levels}[z]$\Comment{Section~\ref{sec:tile-build}}
\EndFor
...
\item recompute the per-tile edge clips on levels $0,\ldots,z$ by
  re-seeding the root tile with the new curves and applying the
  top-down midline split, Section~\ref{sec:tile-build}.
\end{enumerate}
...
\paragraph{Per-level routing with an optional hint.}
For level~$O \le z < Z$ we build a dedicated CDT from the inflated
polylines of~$V_{z}$ only, using each node's level-$z$ scale.  If we
process the levels starting from $Z-1$ then for every edge~$e\in E_z$
we already have a route~$c_e$ computed at the finer
level~$z+1$. Sampling~$c_e$ and listing the level-$z$ CDT triangles it
visits, expanded by one neighbor hop, yields a triangle
\emph{sleeve}~$S_e$ that we can use as a per-edge hint for an A*
search restricted to~$S_e$. This keeps the coarse route close to the
fine one, so an edge deforms smoothly as the user zooms out instead of
ocassionally flipping to the other side of a node obstacle. The price
is one independent A* per edge.
...
We speed up the edge clip generation by the following idea. Consider a
tile and and edge clip with the curve that might intersect the
boundary of the tile only at curve's end points. Intersect the curve
with the middle lines of the tile and consider the new clips obtained
by cutting the curve at these intersectieons. The new clips have the
same property : the only overlap the boundary of the new tiles at
their boundaries.
When we create the level we have each edge clip already contained in
its tile, so the subdivision starts from it.
When we prepare the tiles for rendering we have graph $G_z$ with brand
new edges of the level, so for each edge of $G_z$ we start the subdivision with the top level
tile. 

\paragraph{Bundle merge.}
To correctly reflect the number of element in a tile and speed up the
edge clip calculation even more, the clips with the coinciding end
points are bundled. Here we use the fact that the sleeve router is
deterministic and it finds the unique shortest path between the two
end points. Table~\ref{tab:max-per-tile} reports the resulting maximum
...
The renderer supports pan, zoom, hover highlighting, and click
selection. Hovering an edge highlights it and its incident nodes. The
\texttt{zoomTo} API animates the viewport to a target rectangle with
smooth transitions. Hoveringn on the nodes highglights the node, its
one hope neighbors, and two-hope neigbors in different colors.
