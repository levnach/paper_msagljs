...
selectively to the corresponding node center. Walking each chain
outward from~$s$, we locate the last source-owned vertex at which the
chain bends around~$s$ rather than moving away from it, and replace
every source-owned vertex on that chain up to and including that one
by~$s$; the target end is processed mirror-symmetrically. Diagonals
whose two endpoints coincide after collapse are dropped, and the
left/right orientation of each surviving diagonal is preserved from
the uncollapsed geometry.
