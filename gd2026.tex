...
box. Every level keeps a prefix of rank-ordered graph's nodes the
edges with both endpoints in the prefix.
...
we do not `lose` any elements. We continue increasing $Z$ generating
new levels until a stopping conditions is met. Then, while keeping the
finest level intact, we clean up the tiles of lelels $0, ..., Z-1$ and
populate them for rendering.

We start from $Z=0$, and while the tiles at level~$Z$ violate
...
elements, with default $\mathcal{C}=500$; and (ii)~the tile width and
height are both below ten times the average node width and height. We
additionally enforce a hard cap $Z \le Z_{\max}$ derived from a memory
estimate. At level~$z$ the pyramid has a $K = 2^{z}\times 2^{z}$
tiles. Across the whole pyramid we estimate the total number of stored
tile elements as
\[
...
Section~\ref{sec:tile-build}. Figure~\ref{fig:tile-pyramid} shows two
adjacent pyramid levels built by this procedure.
