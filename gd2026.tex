...
\For{$z = Z-1$ \textbf{down to} $0$}
  ...
  \State route every edge $e\in E_z$ on this CDT, optionally using the level-$(z{+}1)$ route as a sleeve hint \Comment{Section~\ref{sec:edge-hint}}
...
idea. Consider a tile and an edge clip with the curve that intersects
the boundary of the tile only at curve's endpoints. Cut the curve with
the middle lines of the tile and consider the new clips obtained this
way. They have the same property: all intersections of the smaller
tile boundaries and the new edge clips happen at the endpoints.

When we create the level's pyramid, line~\ref{algln:split} of
Algorithm~\ref{alg:build-pyramid}, we have each edge clip already
contained in its tile, so the subdivision starts from it.  However,
when we prepare the tiles for rendering, line~\ref{algln:rebuild} of
Algorithm~\ref{alg:build-pyramid}, we have graph $G_z$ with rerouted
edges of the level, so for each edge of $G_z$ we start the subdivision
by putting the edge curve in an empty tile containing the whole graph.

\paragraph{Bundling edge clips.}
To correctly reflect the number of elements in a tile and speed up the
edge clip calculation even more, clips within a tile whose two
endpoints coincide are bundled into a single clip whose attached edge
list accumulates every contributing edge; the bundled geometry is
taken from the first occurrence. Substituting one short curve for
another with matching endpoints also adds visual stability.
...
(407 nodes, 2\,639 edges) it cuts routing time by $\sim$36\,\%.
Table~\ref{tab:dijkstra-vs-astar} reports the gain across the full
benchmark suite.
...
pass by $\sim$22\,\% and $\sim$37\,\% respectively. Table~\ref{tab:vc-vs-dijkstra}
reports the gain on the full benchmark suite.
...
Tables~\ref{tab:dijkstra-vs-astar} and~\ref{tab:vc-vs-dijkstra} isolate
the contribution of the two source-side optimizations of
Section~\ref{sec:sleeve}: batched Dijkstra trees over per-edge A* and
the vertex-cover heuristic on top of that. Each row times a single
sleeve-search pass run after layout, with the constrained Delaunay
triangulation excluded so that only the search costs are compared. The
batched Dijkstra trees beat per-edge A* on every social graph, with
the gain growing with the average degree and reaching $+71$\,\% on
\texttt{facebook\_combined} and $+63$\,\% on \texttt{ca-HepPh}; the
vertex-cover heuristic then shaves a further $12$--$47$\,\% off those
same graphs. The Delaunay mesh \texttt{delaunay\_n15} is the only
outlier: its source set is almost in bijection with its vertices, so
neither optimization has many opportunities to share paths and the
bookkeeping leaves them marginally slower than the baselines.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrrrr@{}}
    \toprule
    Graph & $|V|$ & $|E|$ & A* (s) & Dijkstra (s) & Speedup \\
    \midrule
    gameofthrones      &     407 &   2\,639 &   0.22 &   0.14 & $+36$\,\% \\
    composers          &  3\,405 &  13\,832 &   8.71 &   5.96 & $+32$\,\% \\
    ca-GrQc            &  5\,242 &  28\,968 &   7.17 &   6.33 & $+12$\,\% \\
    facebook\_combined &  4\,039 &  88\,234 &  45.82 &  13.29 & $+71$\,\% \\
    ca-HepTh           &  9\,877 &  51\,946 &  56.98 &  46.35 & $+19$\,\% \\
    ca-HepPh           & 12\,008 & 236\,978 & 362.0  & 135.5  & $+63$\,\% \\
    ca-CondMat         & 23\,133 & 186\,878 & 576.7  & 379.6  & $+34$\,\% \\
    deezer\_europe     & 28\,281 &  92\,752 & 526.9  & 470.1  & $+11$\,\% \\
    delaunay\_n15      & 32\,768 &  98\,274 &  26.08 &  31.59 & $-21$\,\% \\
    \bottomrule
  \end{tabular}
  \caption{Sleeve-search time of batched Dijkstra trees grouped by
    source vs.\ per-edge A*\@. The CDT-construction phase, identical
    in both modes, is excluded so that only the search costs are
    compared. \emph{Speedup}~$=$~$(\text{A*}-\text{Dijkstra})/\text{A*}$.}
  \label{tab:dijkstra-vs-astar}
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrrrr@{}}
    \toprule
    Graph & Sources & VC roots & Dijkstra (s) & Dijkstra+VC (s) & Speedup \\
    \midrule
    gameofthrones      &     331 &     198 &   0.14 &   0.11 & $+22$\,\% \\
    composers          &  2\,645 &  1\,281 &   5.96 &   3.78 & $+37$\,\% \\
    ca-GrQc            &  5\,241 &  2\,796 &   6.33 &   3.87 & $+39$\,\% \\
    facebook\_combined &  3\,663 &  3\,045 &  13.29 &  11.65 & $+12$\,\% \\
    ca-HepTh           &  9\,875 &  5\,003 &  46.35 &  26.94 & $+42$\,\% \\
    ca-HepPh           & 12\,006 &  7\,032 & 135.5  &  91.7  & $+32$\,\% \\
    ca-CondMat         & 23\,133 & 13\,564 & 379.6  & 200.4  & $+47$\,\% \\
    deezer\_europe     & 21\,060 & 13\,450 & 470.1  & 375.8  & $+20$\,\% \\
    delaunay\_n15      & 28\,467 & 23\,456 &  31.59 &  32.08 & $-2$\,\% \\
    \bottomrule
  \end{tabular}
  \caption{Vertex-cover gain over plain Dijkstra-tree routing.
    \emph{Sources} is the number of distinct edge endpoints in the
    demand graph; \emph{VC roots} is the number of Dijkstra trees
    actually launched after the greedy maximum-degree vertex-cover
    reorientation of Section~\ref{sec:sleeve}. \emph{Speedup}~$=$~$(\text{Dijkstra}-\text{Dijkstra+VC})/\text{Dijkstra}$.}
  \label{tab:vc-vs-dijkstra}
\end{table}
