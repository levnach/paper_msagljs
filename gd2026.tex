...
\State $\mathit{levels} \gets [\,T_0\,]$;\quad $z \gets 0$;\quad $\mathit{used} \gets |T_0|$ \Comment{$|T|$ = element count of tile $T$, summed over all built levels}
\While{some tile in $\mathit{levels}[z]$ exceeds $\mathcal{C}$ elements \textbf{and} its width/height $\ge \mathit{minTileSize}$}
  \State $\mathit{next} \gets$ split every tile of $\mathit{levels}[z]$ into four sub-tiles
  ...
  \If{$200\cdot(\mathit{used} + |\mathit{next}|) > \mathcal{M}_{\max}$}
    \State \textbf{discard} $\mathit{next}$; \textbf{break} \Comment{memory budget reached; check is incremental during the split}
  \EndIf
  \State $\mathit{levels}[z{+}1] \gets \mathit{next}$;\quad $\mathit{used} \mathrel{+}= |\mathit{next}|$;\quad $z \gets z + 1$
\EndWhile
...
The single tile at level~$0$ has a rectangle equal to the graph
bounding box. Each coarser level keeps a prefix of the
PageRank-ordered nodes together with all edges whose endpoints both
lie in that prefix; the finest level keeps the full node and edge
sets.
...
this phase was never a bottleneck: the per-level rerouting of
Section~\ref{sec:level-reroute} dominates the build time at every
level we measured.
...
Once each level $G_z$ has its final geometry, we cut that level into the tiles defined at the start of Section~\ref{sec:tiling}; we write $(\mathit{rect}, \mathit{tiledata})$ for a tile, with $\mathit{rect}$ its rectangle. Two edge clips inside the same tile are merged whenever their curves coincide on the tile's interior, with the second clip's edge list appended to the first's; the implementation detects coincidence by snapping each clip's tile-entry and tile-exit points to the layout tolerance and matching the resulting endpoint pairs. Because the sleeve router is deterministic, parallel edges sharing endpoints route along the same curve, so the merger is exact in the dense-supernode case it is most needed for.

The single tile at level~$0$ is obtained by clipping every element to the graph's bounding box. To build level~$z+1$ from level~$z$, each tile is split into four equal sub-tiles; nodes, labels, and arrowheads are assigned to sub-tiles by bounding-box intersection, and each edge clip is cut at the tile midlines into sub-clips confined to individual sub-tiles, each sub-clip inheriting the parent's edge list. We maintain invariant~$\mathcal{F}$: (a)~every clip stays inside its tile's rectangle, crossing the boundary only at its endpoints; (b)~for every edge~$e$, the union of all clips whose edge list contains $e$ inside a tile equals $c_e \cap \mathit{rect}$. Part~(a) makes subdivision cheap and local: thanks to~(a) we never need to clip against the four outer sides of a tile, only against its two midlines. For the default polyline edges produced by the sleeve router this is trivial---each segment meets each midline at most once---so a clip is split by walking its segments and cutting at the midline crossings. The optional Bezier-corner pass of Section~\ref{sec:collapse} replaces these line--line intersections with curve--line ones plus a parameter sort, but obeys the same invariant. Part~(b) is the rendering-correctness condition: at any zoom only the tiles intersecting the viewport are fetched and their clips drawn, and (b) guarantees that the union of what is drawn equals $c_e\cap\text{viewport}$ for every visible edge~$e$. The same invariant makes the rerouting of Section~\ref{sec:level-reroute} compatible with tiling: whenever an edge curve is replaced by a coarser-level route, (b) is reestablished by recomputing the affected clips.
...
\begin{table}[t]
  ...
  \caption{End-to-end loading benchmarks on an Apple M4 Max laptop, all times
    ...
    rerouting; \emph{Total} is the total loading time including
    parsing of the graph file, graph layout, and all other stages.}
  \label{tab:perf}
\end{table}
