...
The closest precedent to our routing pipeline is the CDT-and-funnel pathfinder of Demyen and Buro~\cite{demyen2006efficient}, originally designed for game-AI units of nonzero radius. They run admissible A* search on the CDT dual (with a reduced-graph variant that collapses corridor chains into single abstract edges) to recover provably shortest paths. Their search is, however, per-query: the bounds depend on the start and goal, so it cannot share work across the many edges leaving a common source as our per-source Dijkstra does. For the graphs we target this would be roughly an order of magnitude slower; we therefore trade their proven optimality for batched throughput suitable for browser-side rerouting at every zoom level.


%% ═══════════════════════════════════════════════════════════════════
\section{Loading a Graph}

The loading of a graph with the WebGL renderer starts with the initial
layout and routing is done for the whole graph, and then the tiling
process. For this method to be visually stable we use IPSep-CoLa
layout followed by sleeve routing, see
...
Next the renderer described in Section~\ref{sec:webgl} consumes the tile hierarchy and renders the appropriate level of detail based on the current viewport.
...
\begin{figure}[!tb]
  ...
  \end{tikzpicture}}
  \caption{The entities rendered on each level as the user zooms in.}
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrrrr@{}}
    ...
    \midrule
    gameofthrones      &     407 &   2\,639 &   0.17 &   0.22 &   1.94 \\
    composers          &  3\,405 &  13\,832 &   4.37 &   5.20 &  10.39 \\
    ca-GrQc            &  5\,241 &  14\,484 &   4.01 &   3.15 &   8.42 \\
    ca-HepTh           &  9\,875 &  25\,973 &  26.83 &  15.95 &  46.07 \\
    ca-HepPh           & 12\,008 & 236\,978 &  82.10 & 156.60 & 246.30 \\
    facebook\_combined &  4\,039 &  88\,234 &  12.25 &  29.87 &  43.82 \\
    ca-CondMat         & 23\,133 & 186\,878 & 224.20 & 188.10 & 431.20 \\
    deezer\_europe     & 28\,281 &  92\,752 & 384.20 & 143.00 & 552.20 \\
    delaunay\_n15      & 32\,768 &  98\,274 &  31.95 &  16.08 &  76.30 \\
    \bottomrule
  \end{tabular}
