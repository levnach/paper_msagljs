...
pass by $\sim$28\,\% and $\sim$31\,\% respectively. Table~\ref{tab:routing-modes}
...
\begin{table}[t]
  ...
  \caption{End-to-end loading benchmarks in headless Chrome~148 on an
    Apple~M4~Max laptop, all times in seconds. \emph{Routing} sums the
    CDT-construction and Dijkstra-tree sleeve-search phases of
    Section~\ref{sec:sleeve} on the full graph; \emph{Tiling} is the
    build of the tile pyramid (capacity $\mathcal{C}=500$,
    $Z_{\max}=8$) of Section~\ref{sec:tiling}, which includes the
    per-level rerouting; \emph{Total} is the total loading time
    including parsing of the graph file, graph layout, and all other
    stages.}
  \label{tab:perf}
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrrrrrr@{}}
    ...
    Graph & Srcs & VC & A* & Dijk. & $\text{VC}_\text{DJ}$ & Dijk./A* & VC/Dijk. \\
    \midrule
  ...
  \caption{Sleeve-search times of the three routing modes on the
    benchmark graphs of Table~\ref{tab:perf}, in headless Chrome~148.
    \emph{Srcs} is the number of distinct edge endpoints in the demand
    graph; \emph{VC} is the number of Dijkstra trees actually launched
    after the greedy maximum-degree vertex-cover reorientation of
    ...
    \emph{VC/Dijk.}~$=$~$(\text{Dijk.}-\text{VC}_\text{DJ})/\text{Dijk.}$\@.}
