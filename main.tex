...
It runs on the client desktop or on a phone, and renders the graph in a web browserowser.
...
\section{Conclusion}
The first contribution is an efficient edge routing algorithm. The algorithm replaces each polyline path with the shortest path that does not intersect the nodes and stays in the polyline homotopy class, the channel, and then produces the smooth composite curve. It uses a modifation of the 'funnel' in the simple polygon algorithm, where we drop the requirement that the polygon is simple.
...
The second contribution is the tiling method that enables large graphs to be visualized in a web browser in a way that is similar to online maps. The method provides an overview of the most important nodes and edges of the graph by making them visible on the top levels while keeping the number of visible elements under a given limit. The novelty of the method is that
