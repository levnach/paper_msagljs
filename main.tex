...
Still, the tile calculation is memory intensive. The largest graph from the table~\ref{tab:perf} that we were able to load with Chrome, and Edge browsers was ~\cite{gnutella}. One of the reasons was the memory limit on a process in those browsers.
\subsubsection*{Edge bundling}
A possible optimization would be finding the repeated segments in the edge curves that naturally happens 
while routing through the same graph with the same algorithm.
