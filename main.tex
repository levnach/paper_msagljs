...
We route a path on the spanner, as illistrated by Fig.~\ref{fig:non_opt_path_L}. Let us call this path \unpath. We are going to find a polygon \plg~ containing \unpath~ and suitable to apply the funnel algorithm mentioned above.\\
\begin{figure}[!tbp]
  \centering
...
To apply the funnel algorithm~\cite{chazelle1982theorem,hershberger1994computing}, we need to have a triangulation of \plg~ such that every edge of the triangulation is either a boundary edge of \plg, or a diagonal of \plg. If \triset~ has this property we use it, otherwise we create a new Constrained Delaunay Triangulation of \plg, where the set of constrained edges is the boundary of \plg, see Fig.~\ref{fig:new_triangulation}.\\
...
\begin{figure}[]
  ...
  \caption{\plg~is not simple, and the dotted path is shorter than the dashed one.}
  \label{fig:non_optimal_path}
\end{figure}


The polygon \plg~is not neccesserely simple, as shown in Fig.~\ref{fig:non_optimal_path}.
Here we also have an example when the path that we calculate with the funnel is not neccesserely the shortest path between the obstacles.
