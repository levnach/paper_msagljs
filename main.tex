...
\begin{figure}[!tbp]
  ...
  \end{minipage}
\end{figure}
Let $\mathcal{S}$ and $\mathcal{E}$ be the obstacles containing, correspondently, \unpath's start and end point.
...
\section{Conclusion}
The first contribution is an efficient edge routing algorithm. In XJS the algorithm starts with routing on the spanner, 
but instead any routing with polylines outside the nodes can be used as the first step for our method.
The algorithm replaces each polyline with the shortest polyline that does not intersect the nodes 
and stays in the polyline homotopy class, the channel, and then produces the smooth composite curve.
The algorithm is fast and produces visually pleasing results.

The second contribution is a tiling method that enables large graphs to be visualized in a web browser in a way that is similar to online maps. The method provides an overview of the most important nodes and edges of the graph by making them visible on the top levels while keeping the number of visible elements under a given limit. The novelty of the method is that
it subdivides a smooth curve using tiles, and incorporates the edge routing algorithm, the 'funnel' in channel heuristic, 
to simplify the edges on the upper levels.
