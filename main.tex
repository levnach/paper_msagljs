...
%Our tool has several layout methods, but we do not describe them here. It is important that they work fast and produce outputs without node overlaps.

%We describe our edge routing algorithm first, and then discuss the tiling method.
%Our edge routing starts on the method of~\cite{dwyer2010fast} and improves it.

% %In addition, we bundle edges per-tile as an optimization heuristic.
...
Next we create curve clips $(e, l_k)=(e, \textit{trim}(p, u_k, u_{k+1}))$, as shown in Fig~\ref{fig:clip}. We assign each curve clip $(e,l_k)$ to the sub-tile with the rectangle containing the bounding box of $l_k$.

Because, by the induction assumption property $\mathcal{F}$ is true on $L_z$, and by construction, each new curve clip can cross the boundary of the sub-tile only at the clip endpoints. We also cover all the intersections of $p$ with the sub-tiles with the new edge clips, so the property $\mathcal{F}$ holds for $L_{z+1}$.
\begin{figure}[!tbp]
  \centering
...
{gnutella}. One of the reasons was the memory limit on a process in those browsers.
...
\begin{algorithm}
  ...
  \end{algorithmic}
\end{algorithm}\\
Here removeEntities}($L$) empties all the tiles of layer $L$, but returns map $r$ allowing to restore the tiles. Function \textit{addNodeToLayer}($n$) tries to add the node to $L$, it also tries to add the tile elements for self edges of $n$, and the tile elements for the edges connecting $n$ with the nodes ranked at least as high as $n$. These nodes are the nodes already added to $L$.
...
This gives us simpler edge routes but still has the visual stability during the layer change while browsing.
\subsection*{Node labels visibility}
Cite some papers on label positioning. %find citatitons 

\section{Future work}
\begin{itemize}
\item Find a tiling method that guarantees that each tile has no more than \capac~elements on every level. One approach could be to use a more aggressive edge bundling to reduce the number of edge clips in the tiles.
A simple idea is finding the repeated segments in the edge curves that naturally appear
while routing through the same graph with the same algorithm, and reuse the repeated segment to save memory.
\item 
The tile calculation is memory intensive.
The largest graph from the table~\ref{tab:perf} that we were able to load with Chrome, and Edge using the tiling procedure was ~\cite{gnutella}. One of the reasons was the memory limit on a process in those browsers, another was the long running-time of the tiling procedure. A possible measure would be saving the tiles to the disk and loading them on demand.
\item
