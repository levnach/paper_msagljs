...
\begin{algorithm}
  ...
  \end{algorithmic}
\end{algorithm}\\
Here removeEntities}($L$) empties all the tiles of level $L$, and returns map $r$ allowing to restore the tiles. Function \textit{addNodeToLevel}($n$) tries to add the node to $L$, it also tries to add the tile elements for self edges of $n$, and the tile elements for the edges connecting $n$ with the nodes ranked at least as high as $n$. These nodes are the nodes already added to $L$.
