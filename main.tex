...
An application of the 'path in a simple polygon' optimization to edge routing is not a new approach.
The authors of Graphvis used it~\cite{dobkin1997implementing}, but only for hierarchical layouts, where a simple polygon, \plg, containing the path is available. They write: "If \plg~does not contain holes ... we can apply a
standard “funnel” algorithm~\cite{chazelle1982theorem,hershberger1994computing}
for finding Euclidean shortest paths in a simple polygon". In general case,
for a non-layered layout, they build the visibility graph which is
very expensive.

In our settings we are able to find the polygon \plg~even for any layout. We drop the requirement that \plg~is simple. Indeed, to run the “funnel” algorithm one only needs a “sleeve”: a sequence of triangles leading from the start to the end of the path, where each triangle shares an edge with its successor. Let us show how to build polygon \plg, create a sleeve, and produce an optimized path.
...
Two parameters controll the algorithm: tile capacity, \capac~, the maximum number of elements visible in a tile, and the minimala size of a tile: (\mw, \mh). The elements or a tile are curve clips, arrowheads, nodes, or edge labels. In our setting \capac~$=500$, and $(\mathcal{W},\mathcal{H}) = 3(w,h)$, where $w$ is the average width and $h$ is the average height of the nodes of the graph.


The first phase starts with $L(0) = \{(0,0) \rightarrow \textit{tile data}\}$: and \textit{tile data} comprising curve clips $(e,c(e))$, where $e$ is an edge of the graph, all graph nodes, all edge labels, and all edge arrowheads. If the number of these elements is not greater than \capac~the first phase, and the whole algorithm, stop; this is the usual case for a small graph.

Otherwise, the first phase continues. Let us suppose that level $z$ is built. We denote by $C_z(i,j)$ the number of elements in $L(z)(i,j)$. If for each $(i,j)$ we have $C_z(i,j) \leq$ \capac, then the first phase stops and the second phase starts. The same happens if the $w \leq \mathcal{W}$ and $h \leq \mathcal{H}$, where $w$ ($h$) is half length of the current tile width (correspondengly, height). 

Otherwise, we proceed to build the next level $z+1$.
We divide each tile $t=L(z)(i, j)$ into for equal sized sub-tiles. For each node, arrowhead, or edge label of $t$ if the element's bounding box intersects a subtile then we assign the element to it.

The edge clip treatment is more involved. Let $ab$ be the horizontal midline segment of $t$, and $cd$ be the vertical midline segment of $t$. Let $(e, c)$ be a curve clip of $t$. We find all intersections of curve $c$ with $ab$ and $cd$. Each $j$-th intersection can be represented as $c[t_j]$. We sort sequence $[s, \dots, t_j, \dots, e]$ in the ascending order, obtain sequence $u$. 
Here $s,e$ is the parameter domain of $c$.
We obtain curve clips $(e, \text{trim}(c, u_k, u_{k+1}))$ for $k = 0, \dots, n-1$, where $n$ is the length of $u$. We assign each curve clip to the subtile containing its bounding box. By the assumption above, and by construction, each new curve clip can cross the boundary of the subtile only at the clip endpoints.
