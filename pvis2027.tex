...
the subsequent per-level rebuilding. In out experimenst on every
...
\label{sec:filter}
Before filling the level's tiles for rendering we sort elements of $V$
by PageRank~\cite{page1999pagerank} in descending order. From now on
we think of $V$ as a sorted array.
%&The nodes populating level $z$ belong to the prefix of
%$V$ of length $\lceil |V|/2^k\rceil$, where $k = Z - z$. Let $V_z$ be
%the nodes accepted from this prefix by the overlap filter described
%below. The level graph is $G_z=(V_z,E_z)$, where $E_z=\{uv\in
%E(G):u,v\in V_z\}$: only the edges connecting two level nodes are
%included into the level as well.
...
We add nodes to level $z$ by walking the first
$\lceil|V|/2^{Z-z}\rceil$ nodes of $V$. We do not change the node
positions inherited from the finest level but try to increase the
...
All nodes are present at the finest level.

The edges of the level are those connecting two level nodes.
