...
To choose $Z$, we build the pyramid without any element
filtering: every level contains all graph entities. Starting from the
root tile, we keep adding levels and splitting tiles until a stopping
condition is met; the index of the finest level built this way is
$Z$. The finest level is kept as is, while the tiles of
levels~$0, \ldots, Z-1$ are later rebuilt as described below.
...
\]
These means are computed once over the full graph, not separately
for each tile or PageRank prefix. The tile side length $\ell_z$ is
measured in the same world-coordinate units.
...
the subsequent per-level rebuilding. On every
...
$\mathcal{C}=500$.
\paragraph{Three phases.} Once $Z$ and the tile squares are fixed,
...
length $\lceil |V|/2^k\rceil$. Let $V_z$ be the nodes accepted from
this prefix by the overlap filter below. The level graph is
$G_z=(V_z,E_z)$, where $E_z=\{uv\in E(G):u,v\in V_z\}$; an edge is
excluded if either endpoint is dropped.
...
selected nodes, with the routing padding included in the overlap
test. Each accepted scale lies between $1$ and the preceding
accepted scale. If even at scale~$1$ the candidate's padded bounding
box would overlap an already-accepted node's padded scaled box, the
...
The candidate prefixes are nested, but the accepted sets $V_z$
need not be: overlap filtering is performed independently at each
level. All nodes are present at the finest level.
...
accepted padded box, and any candidate's padded box at the level's
maximum scale, have width and height at most the cell size.
Because of this choice, two
...
\caption{Maximum number of elements rendered in a single tile across
  ...
  holds approximately 5.9--7.3$\times$ the nominal capacity~$\mathcal{C}$.}
...
CDT construction takes about 6\,ms on Game of Thrones and at most
420\,ms in this benchmark. It accounts for about $8\,\%$ of
\emph{Routing} on Game of Thrones and at most $6.4\,\%$ on the other
...
approximately 5.9--7.3$\times$ the nominal $\mathcal{C}$ in the worst cases, which leaves
